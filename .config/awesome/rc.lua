...
mytextclock = wibox.widget.textclock(" %a %d %b, %H:%M ", 2)
...
-- wp_timeout  = 900
-- wp_path = "/home/phrb/Dropbox/Wallpapers/"
...
--         gears.wallpaper.maximized(wp_path .. wp_files[math.random( #wp_files )], s, false)
--     end
--     wp_timer:stop()
...
awful.screen.connect_for_each_screen(function(s)
    ...
    awful.tag({ "a", "b", "c", "d", "e"}, s, awful.layout.layouts[3])
...
-- Changing teams notifications.
naughty.config.presets.teams = {
    -- if you want to disable Spotify notifications completely, return false
    callback = function(args)
        return true
    end,
...
table.insert(naughty.dbus.config.mapping, {{appname = "Microsoft Teams"}, naughty.config.presets.teams})

-- Changing calibre notifications.
naughty.config.presets.calibre = {
    -- if you want to disable calibre notifications completely, return false
    callback = function(args)
        return true
    end,
...
table.insert(naughty.dbus.config.mapping, {{appname = "calibre"}, naughty.config.presets.calibre})
